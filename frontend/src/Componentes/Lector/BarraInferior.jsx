// src/Componentes/Lector/BarraInferior.jsx - VERSIÓN SIMPLIFICADA
import React, { useState, useCallback } from 'react';
import './barraInferior.css';

/**
 * Barra de controles inferior del lector PDF simplificada
 * Solo incluye: navegación, zoom y pantalla completa
 */
export default function BarraInferior({ 
  ...
  /**
   * Maneja el input directo de página
   */
  const manejarCambioPagina = useCallback((event) => {
    if (event.key === 'Enter') {
      ...
      if (numeroPagina >= 1 && numeroPagina <= totalPaginas) {
        ...
        // Revertir a página actual si número inválido
  ...
  /**
   * Sincronizar input de página con prop
   */
  React.useEffect(() => {
    setInputPagina(paginaActual.toString());
  }, [paginaActual]);
